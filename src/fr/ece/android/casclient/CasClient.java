 ...
 * HttpComponentsClient4 and Log4J, and adapted to work with the ECE CAS server.
 * 
...
public class CasClient {
	 ...
	 * providing authentication services. If you use
	...
	/**
	 * Construct a new empty CasClient which is use to instantiate the singleton
	 */
	public CasClient() {
	 ...
	 *            "login" to the CAS.
	 * @param casBaseUrl
	 *            The base URL of the CAS service to be used. If you use
	 *            <code>http://x.y.z/a/login</code> to login to your CAS, then
	 *            the base URL is <code>http://x.y.z/a/"</code>.
	 */
	...
	public String login(String serviceUrl, String username, String password)
		...
		// The login method simulates the posting of the CAS login form. The
		// login form contains a unique identifier
		// or "LT" that is only valid for 90s. The method getLTFromLoginForm
		// requests the login form from the cAS
		// and extracts the LT that we need. Note that the LT is _service
		// specific_ : We need to use an identical
		...
		if (lt == null) {
			...
			// Yes, it is necessary to include the serviceUrl as part of the
			// query string. The URL must be
			...
			try {
				...
				// authenticated, the CAS server
				// redirects transparently to the service URL!
				// Success if CAS replies with a 302 HTTP status code and a
				// Location header
				// We assume that if a valid ticket is provided in the Location
				// header, that it is also a 302 HTTP STATUS
				...
				if (serviceTicket == null) {
					Log.i(TAG, "Authentication to service '" + serviceUrl
							+ "' unsuccessul for username '" + username + "'.");
					...
					Log.i(TAG, "Authentication to service '" + serviceUrl
							+ "' successul for username '" + username + "'.");
			} catch (IOException e) {
	...
	/**
	 * Logout from the CAS. This destroys all local authentication cookies and
	 * any tickets stored on the server.
	 * 
	...
	/**
	 * Shuts down the HttpClient's connection. Use this to release all resources
